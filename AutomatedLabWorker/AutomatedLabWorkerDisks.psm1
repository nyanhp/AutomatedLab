...
#region New-LWReferenceQcow 
function New-LWReferenceQcow 
{
    [CmdletBinding()]
    Param (
        #ISO of OS
    ...
    try
    {
        ...
        Write-PSFMessage "Mounting ISO image '$IsoOsPath'"
        [void] (Mount-LabDiskImage -ImagePath $IsoOsPath -StorageType ISO)
        ...
        $vmDisk = qemu-img create -f qcow2 $ReferenceVhdxPath "$($SizeInGB)G"
        Write-PSFMessage "Created qcow2 file '$($vmDisk.Path)'"

        Write-ScreenInfo -Message "Creating base image for operating system '$OsName'" -NoNewLine -TaskStart

        if (-not (lsmod | Select-String -Pattern 'nbd'))
        {
            # Attempt to insert nbd module. max_part 8 - 8 partitions per device
            modprobe nbd max_part=8
        }

        $null = qemu-nbd --connect=/dev/nbd0 $ReferenceVhdxPath

        if (-not (Test-Path -Path '/mnt/automatedlab/refdisk'))
        {
            $null = New-Item -ItemType Directory -Force -Path '/mnt/automatedlab/refdisk'
        }
