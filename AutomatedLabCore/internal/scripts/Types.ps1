...
try {
    ...
    $typeAvailable = try { [System.Security.Cryptography.X509Certificates.X509CertificateLoader] } catch {}
    if ($typeAvailable) {
        Add-Type -TypeDefinition $pkiInternalsTypes
    }
    else {
        Add-Type -TypeDefinition $pkiInternalsTypesLegacy
    }
}
