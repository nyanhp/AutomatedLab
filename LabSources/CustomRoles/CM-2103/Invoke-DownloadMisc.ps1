...
try
{
    $ncli = Get-LabInternetFile -Uri $URL -Path (Split-Path -Path $SQLNCLIMSIPath) -FileName (Split-Path $SQLNCLIMSIPath -Leaf) -ErrorAction "Stop" -ErrorVariable "GetLabInternetFileErr" -PassThru
}
catch
{
...
Install-LabSoftwarePackage -Path $ncli.FullName -ComputerName $ComputerName -CommandLine "/qn /norestart IAcceptSqlncliLicenseTerms=Yes" -ExpectedReturnCodes 0
...
{
    $WMIv2Zip = "{0}\WmiExplorer_2.0.0.2.zip" -f (Get-LabSourcesLocation -Local)
